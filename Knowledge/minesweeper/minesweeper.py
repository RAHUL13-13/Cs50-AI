...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for n in range(max(0, i - 1), min(i + 2, self.height)):
            for k in range(max(0, j - 1), min(j + 2, self.width)):
                if (n, k) != (i, j):
                    neighbors.add((n, k))
        ...
        for sentence1 in self.knowledge:
            if len(sentence1.cells) != 0 and sentence1.count != 0:
                for a in self.knowledge:
                    if len(a.cells) != 0 and a.count != 0:
                        if a.cells.issubset(sentence1.cells):
                            g = copy.deepcopy(a)
                            if (g.count - sentence1.count) == 0:
                                new = set()
                                for cell in g.cells:
                                    if cell not in sentence1.cells:
                                        self.mark_safe(cell)
                                        new.add(cell)
                                sent_new = Sentence(new, 0)
                                if sent_new not in self.knowledge:
                                ...
                                sent_new = Sentence(new, (sentence1.count - g.count))
                                if sent_new not in self.knowledge:
                                    self.knowledge.append(sent_new)
            ...
            if len(sentence1.cells) == sentence1.count:
                if sentence1.count != 0:
                    if len(sentence1.cells) != 0:
                        p = copy.deepcopy(sentence1)
                        for cell in p.cells:
                            self.mark_mine(cell)

        for sentence1 in self.knowledge:
            # if any number of cells cell and count is 0 then its mine:
            if sentence1.count == 0:
                if len(sentence1.cells) != 0:
                    w = copy.deepcopy(sentence1)
                    for cell in w.cells:
                        self.mark_safe(cell)

        for sentence1 in self.knowledge:
            if sentence1.count == 1:
                if len(sentence1.cells) == 1:
                    w = copy.deepcopy(sentence1)
                    for cell in w.cells:
                        self.mark_mine(cell)
